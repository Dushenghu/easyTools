package com.du.easytools.common.auto;
...
public class easyAuto {
     ...
     **     Cron表达式的格式    **
     ...
     符号“*”在“/”前面，其本质：表达式的每个数值域都是一个有最大值和最小值的集合，如：秒域和分钟域的集合是0-59，日期域是1-31，月份域是1-12。
     ...
     如果是在日期域填写“1W”即使1号是周六，那么任务也只会在下周一，也就是3号触发，“W”字符指定的最近工作日是不能够跨月份的。字符“W”只能配合一个单独的数值使用，不能够是一个数字段，如：1-15W是错误的。
     “L”和“W”可以在日期域中联合使用，LW表示这个月最后一周的工作日。

     “#”
     字符“#”只允许在星期域中出现。这个字符用于指定本月的某某天。
     例如：“6#3”表示本月第三周的星期五（6表示星期五，3表示第三周）。“2#1”表示本月第一周的星期一。“4#5”表示第五周的星期三。
     ...
     **/
    public static String generateCron(String say){
        System.out.println("表达格式：秒数-分钟-小时-日期-月份-星期-年份(可为空)，当输入 END 时，结束表达。");

        List<String> formate = new ArrayList<String>();
        formate.spliterator();
        List<String> says = new ArrayList<String>();

        while (!"END".equals(say)){
